...
public class CurrencyTest {
    ...
    public void shouldFindAllGamesUsingJpqlQuery() throws Exception {
        // given
        String fetchingAllUsersInJpql = "select u from Currency u order by u.id";

        // when
        System.out.println("Selecting (using JPQL)...");
        List<Currency> currencies = em.createQuery(fetchingAllUsersInJpql, Currency.class).getResultList();

        // then
        System.out.println("Found " + currencies.size() + " currencies (using JPQL):");
        Assert.assertTrue(currencies.get(0).getSymbol().equals("USD"));
    }
